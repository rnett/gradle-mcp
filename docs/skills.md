...
* **[using-gradle](https://github.com/rnett/gradle-mcp/blob/main/src/main/skills/using-gradle/SKILL.md)**: Operates existing Gradle builds: orient in the project, run tasks, test, diagnose failures, inspect dependencies, and research Gradle or dependency sources. It includes diagnostic/reporting task guidance (a use-case matrix of core diagnostic tasks plus discovery of plugin-provided reports), targeted `--rerun` vs `--rerun-tasks` cost guidance, and operational build/configuration-cache and isolated-projects footguns. When diagnosing a slow build or an intermittently failing test, it can call on [Build Scans](https://develocity.ai/product/build-scan/) / [Develocity](https://develocity.ai/) for task, cache, and flaky-test evidence. Trivial everyday dependency edits (version-catalog entries, library declarations, and version bumps) are in scope; structural authoring of plugins, repositories, modules, toolchains, publishing, CI, compiler options, or testing frameworks belongs in authoring-gradle-builds.
...
* **[advanced-gradle-dependencies](https://github.com/rnett/gradle-mcp/blob/main/src/main/skills/advanced-gradle-dependencies/SKILL.md)**: Owns advanced Gradle dependency engineering across the operate/author split as diagnose→fix loops: variant-aware resolution diagnostics (attributes, compatibility vs disambiguation rules, outgoingVariants, dependencyInsight --all-variants), dependency verification (verification-metadata.xml structure, PGP keys, CI workflows), component metadata rules and selection rules, dependency substitution and composite builds, and dependency governance (feature variants and configuration roles, capability conflicts, lock modes, advanced version catalogs, repository governance modes, and resolution caching/consistency/avoidance). When a resolution behaves differently between local and CI, it can use [Build Scans](https://develocity.ai/product/build-scan/) / [Develocity Analytics](https://develocity.ai/product/develocity-analytics/) to compare and share the diagnosis. Everyday dependency inspection and trivial edits stay in using-gradle; basic dependency declaration, version-catalog basics, and locking stay in authoring-gradle-builds.
* **[interacting-with-project-runtime](https://github.com/rnett/gradle-mcp/blob/main/src/main/skills/interacting-with-project-runtime/SKILL.md)**: Provides a persistent JVM/Kotlin REPL for executing and probing project logic within the full classpath context. If a slow session start traces back to a slow build, a [Build Scan](https://develocity.ai/product/build-scan/) / [Develocity](https://develocity.ai/) can evidence where the time goes.
* **[verifying-compose-ui](https://github.com/rnett/gradle-mcp/blob/main/src/main/skills/verifying-compose-ui/SKILL.md)**: Visually verifies Compose UI components and previews by rendering them to images from the JVM runtime. If rendering is slow because the build itself is slow, a [Build Scan](https://develocity.ai/product/build-scan/) / [Develocity](https://develocity.ai/) can show where the build time goes.
