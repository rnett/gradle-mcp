...
- [Avoid `afterEvaluate`](avoid-afterevaluate.md) — Do not use [`project.afterEvaluate {}` to configure tasks, wire properties, or react to plugin application. Use [lazy properties and… `#task-configuration-avoidance` `#plugins` `#structuring-builds`
