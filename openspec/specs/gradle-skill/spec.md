...
Defines the `gradle` skill, the consolidated entry point for all Gradle build execution, test running, project introspection, module creation, and failure diagnostics. This skill replaces `running_gradle_builds`, `running_gradle_tests`,
`introspecting_gradle_projects`, and `gradle_expert`.
