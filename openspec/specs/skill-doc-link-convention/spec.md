# Capability: skill-doc-link-convention

## Description
Governs how authored skill content and references link to Gradle documentation, ensuring every documentation link is a clean, canonical `gradle_docs` tool-call hint and that guidance topics carry relevant read-more links.

### Use when
- Authoring or reviewing a `SKILL.md` body or an authored reference that points readers to Gradle documentation.
- Deciding whether a guidance topic warrants a documentation link and which canonical `gradle_docs` form to use.
- Maintaining per-file link structure with inline links and optional trailing authoritative-docs indexes.

## Purpose
This capability defines the canonical documentation-link convention for authored skill content and the coverage expectation for guidance topics, while keeping the generated best-practices corpus confirm-only and outside the canonical-link requirement.
