...
Defines the `exploring_dependency_sources` skill (renamed from `searching_dependency_sources`), which provides guidance for searching and reading source code across external library dependencies, Gradle plugins (buildscript), and Gradle
Build Tool internal source code.
