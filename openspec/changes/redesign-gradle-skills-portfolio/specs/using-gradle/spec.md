...
The skill MUST route agents from every major operating topic to the authoritative version-scoped Gradle documentation and to the published documentation for the related Gradle MCP tools, using only verified references and never fabricated URLs or tool names.
...
- **THEN** the relevant local reference provides a version-scoped `gradle_docs` tag and path hint (resolved to the project's Gradle version) plus the corresponding published `docs.gradle.org/current/userguide/<path>.html` URL
- **AND** the agent reads `gradle/wrapper/gradle-wrapper.properties` before applying version-sensitive advice

#### Scenario: Use a related MCP tool
- **WHEN** an agent is ready to execute, monitor, inspect dependencies, look up Maven releases, search documentation, or read dependency sources
- **THEN** the relevant local reference points to the tool's published page under `https://gradle-mcp.rnett.dev/latest/tools/` using the verified published documentation filename
- **AND** the pointer identifies the actual MCP tool rather than a generic or unpublished reference
...
- **THEN** the references preserve local cross-links and the authoritative documentation and MCP-tool pointers remain available at the topic's procedure home
