...
MUST implement `verifySkillsMaterialized` to gate the `check` task by verifying shared fan-out, generated content hashes, and index completeness.

#### Scenario:
A developer manually edits a materialized file in a skill root instead of the source; `verifySkillsMaterialized` detects the drift from the authoritative source and fails the build.
