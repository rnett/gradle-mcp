# Capability: jdk-source-resolution

## Purpose

Enable AI agents to read and search JDK standard library source code seamlessly when working with JVM-backed Gradle scopes, by detecting the project's configured JDK from the init script, locating the JDK's `src.zip`, extracting it, caching
it using content-addressed storage (SHA-256 hash of `src.zip`), and lazily indexing it for search. JDK sources are auto-included for JVM-backed scopes without requiring any explicit parameter and are exposed in session views under the
reserved `jdk/sources/...` path.
