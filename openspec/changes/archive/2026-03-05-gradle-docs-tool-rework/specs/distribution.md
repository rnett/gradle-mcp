...
1. **Cache check**: If `{cacheDir}/reading_gradle_docs/{version}/gradle-{version}-docs.zip` exists
...
- All kinds except `DSL`: single element — `{cacheDir}/reading_gradle_docs/{version}/converted/{kind.dirName}/`
...
```
{cacheDir}/reading_gradle_docs/{version}/.lock
```
...
2. **Resolve path**: look up `{cacheDir}/reading_gradle_docs/{version}/converted/{path}`.
